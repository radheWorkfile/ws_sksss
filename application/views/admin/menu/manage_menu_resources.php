...
<body class="hold-transition sidebar-mini layout-fixed ">
                            ...
                            <?php foreach ($menu as $mu => $men) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $status = ($men['status'] == 1) ?
                                            "<a style='color:green' href='javascript:void()' onClick='return changeStatus(\"" . $men['id'] . "\",\"0\",\"resource_menu\",\"admin/common/chageStatus\")' title='Click to Di-Active Menu'><b><i class='fa fa-check'></i> </b></a>"
                                            :
                                            "<a style='color:red'  href='javascript:void()'  onClick='return changeStatus(\"" . $men['id'] . "\",\"1\",\"resource_menu\",\"admin/common/chageStatus\")' title='Click to Active Menu'><b><i class='fa fa-times'></i> </b></a>";
                                        echo $status;
                                        ?>&nbsp;
                                        ...
                                        <a href="javascript:void(0);" onclick="delete_data_resources(<?php echo $men['id'] ?>)" title='Click to Delete Menu Details'><i class="fas fa-trash-alt text-danger"></i></a>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
        ...
        <div class="modal fade" id="add_menu_modal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form id="add_resources_data" method="post" accept-charset="utf-8" enctype="multipart/form-data">
        ...
        <div class="modal fade" id="update_menu_modal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form id="edit_resources_data" method="post" accept-charset="utf-8" enctype="multipart/form-data">
    ...
    <script>
        $('#add_resources_data').submit(function(e) {
            e.preventDefault();
            $.ajax({
                url: '<?= base_url() ?>admin/menu/save_menu_resoruces', 
                type: "POST",
        ...
        function update_menu(id) {
            $.ajax({
                url: '<?= base_url() ?>' + 'admin/menu/get_details',
                type: "POST",
                data: {
                    'id': id
                },
        ...
        $('#edit_resources_data').submit(function(e) {
            e.preventDefault();
            $.ajax({
                url: '<?= base_url() ?>admin/menu/update_resources_data',   
                type: "POST", 
                data: new FormData(this),
                processData: false,
                contentType: false,
        ...
        function delete_data_resources(id) {
            if (confirm("Do You Want To Delete")) {
                $.ajax({
                    type: 'POST',
                    url: '<?= base_url() ?>' + 'admin/menu/delete_menu_resources',
                    data: {
                        'id': id,
                    },
                    success: function(data) {
