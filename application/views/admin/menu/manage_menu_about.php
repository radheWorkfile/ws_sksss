...
<body class="hold-transition sidebar-mini layout-fixed ">
                            ...
                            <?php foreach ($menu as $mu => $men) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $status = ($men['status'] == 1) ?
                                            "<a style='color:green' href='javascript:void()' onClick='return changeStatus(\"" . $men['id'] . "\",\"0\",\"menu\",\"admin/common/chageStatus\")' title='Click to Di-Active Menu'><b><i class='fa fa-check'></i> </b></a>"
                                            :
                                            "<a style='color:red'  href='javascript:void()'  onClick='return changeStatus(\"" . $men['id'] . "\",\"1\",\"menu\",\"admin/common/chageStatus\")' title='Click to Active Menu'><b><i class='fa fa-times'></i> </b></a>";
                                        echo $status;
                                        ?>&nbsp;
    ...
    <script>
        $('#add_menu').submit(function(e) {
            e.preventDefault();
            $.ajax({
                url: '<?= base_url() ?>admin/menu/save_data_about',
                type: "POST",
        ...
        function update_menu(id) {
            $.ajax({
                url: '<?= base_url() ?>' + 'admin/menu/get_menu_about',
                type: "POST",
                data: {
                    'id': id
                },
        ...
        $('#menu_updata').submit(function(e) {
            e.preventDefault();
            $.ajax({
                url: '<?= base_url() ?>admin/menu/update_menu_about',
                type: "POST",
                data: new FormData(this),
                processData: false,
                contentType: false,
        ...
        function deletedata(id) {
            if (confirm("Do You Want To Delete")) {
                $.ajax({
                    type: 'POST',
                    url: '<?= base_url() ?>' + 'admin/menu/delete_menu_about',
                    data: {
                        'id': id,
                    },
                    success: function(data) {
